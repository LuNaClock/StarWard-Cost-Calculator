...
*   **予測覚醒ゲージ:** 撃墜後の覚醒ゲージ量を予測。
*   **チーム合計耐久力予測:** 様々な撃墜シナリオでのチーム総耐久力を予測。
*   **ゲーム画面OCR（試験的機能）:** ゲームのスクリーンショットから「耐久値」「覚醒ゲージ」を自動で読み取り、フォームに反映。
...
*   **入力:**
    *   再出撃シミュレーションセクションにある「ゲーム画面から自動入力」ボタンからOCRモーダルを開きます。
    *   「画像をドラッグ＆ドロップまたはクリックして選択」エリアから、スクリーンショット（PNG, JPG等）をアップロードします。
*   **領域の調整:**
    *   画面に表示された画像の上に、「耐久値」と「覚醒ゲージ」と書かれた2つの四角い枠が表示されます。
    *   これらの枠をドラッグ＆ドロップで移動したり、四隅をドラッグしてリサイズしたりして、画像内の数字（耐久値・覚醒ゲージ）を正確に囲むように調整します。
*   **処理:**
    *   「OCR解析開始」ボタンを押すと、調整した枠内の画像を解析します。
    *   認識結果がプレビュー画面に表示されます。上手く認識されない場合は、各プレビュー下にある「ズーム」のスライダーを調整し、再度「OCR解析開始」ボタンを押してください。
*   **結果の適用:**
    *   認識された数値が正しいことを確認したら、「この値をフォームに適用」ボタンを押すと、覚醒ゲージ予測フォームに自動で反映されます。
...
*   **外部ライブラリ:**
    *   **GSAP (GreenSock Animation Platform):** UIのアニメーション演出に使用。
    *   **ONNX Runtime Web:** OCRモデルの推論エンジンとして使用。
    *   **OpenCV.js:** OCRの前処理（画像の切り出しや加工）に使用。
    *   **esearch-ocr:** PaddleOCRをラップしたOCRライブラリ本体。
